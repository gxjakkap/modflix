...
export default function SalesChart({ data, dateFrom, dateTo, series }: SalesChartProps) {
  const title = `Sales Quantity Comparison: ${series.map((s) => s.label).join(" vs ")}`;
  const subtitle = dateFrom && dateTo ? `${dateFrom} – ${dateTo}` : "1 Jan 2568 – 1 Mar 2568";
    ...
    <div className="sales-chart">
      ...
      <div className="chart-legend">
        {series.map((s) => (
          <span key={s.key} className="legend-item">
            ...
            {s.label}
            {s.total !== undefined && <span className="legend-total"> — Total {s.total}</span>}
          </span>
        ))}
      </div>
      <ResponsiveContainer width="100%" height={240}>
        <LineChart data={data} margin={{ top: 8, right: 24, left: -10, bottom: 4 }}>
          ...
          <XAxis dataKey="date" tick={{ fontSize: 11, fill: "#888" }} tickFormatter={(v: string) => v.slice(5)} tickLine={false} />
          <YAxis tick={{ fontSize: 11, fill: "#888" }} domain={[30, 100]} tickLine={false} axisLine={false} />
          <Tooltip content={<CustomTooltip />} />
          {series.map((s) => (
            <Line key={s.key} type="monotone" dataKey={s.key} name={s.label} stroke={s.color} strokeWidth={2}
              strokeDasharray={s.dash ? "6 3" : undefined} dot={{ r: 4, fill: s.color, strokeWidth: 0 }} activeDot={{ r: 6 }} />
          ))}
        </LineChart>
      </ResponsiveContainer>
    </div>
