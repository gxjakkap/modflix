...
async function fetchProducts({ search, page, pageSize }) {
...
export default function LovModal({ open, onSelect, onClose }) {
  ...
  const isAllOnPageSelected =
    sorted.length > 0 && sorted.every((r) => selected[r.code]);
  ...
  // ถ้ามีการเลือก ให้เอาชื่อมาต่อกัน ถ้าไม่มีให้แสดง "Select Product"
  let displayTitle = "Select Product";
  if (selectedCount > 0) {
    if (selectedCount <= 3) {
      // ถ้าเลือกไม่เกิน 3 รายการ ให้แสดงชื่อทั้งหมด
      displayTitle = selectedItems.map((item) => item.name).join(", ");
    } else {
      // ถ้าเลือกเยอะกว่า 3 รายการ ให้แสดงแค่ 2 ชื่อแรก แล้วบอกจำนวนที่เหลือ
    ...
    <div className="lov-backdrop" onClick={onClose}>
      <div className="lov-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="lov-header">
          <h2 className="lov-title" title={selectedItems.map((i) => i.name).join(", ")}>
            {displayTitle}
          </h2>
          <div className="lov-header-right">
            <div className="lov-search-wrap">
              <svg className="lov-search-icon" width="14" height="14" viewBox="0 0 24 24"
            ...
            <div className="lov-pagesize-wrap">
              <select className="lov-pagesize" value={pageSize}
                onChange={(e) => setPageSize(Number(e.target.value))}>
                {PAGE_SIZE_OPTIONS.map((n) => (
                  <option key={n} value={n}>{n} / page</option>
                ))}
              </select>
              <svg className="lov-select-arrow" width="10" height="10" viewBox="0 0 24 24"
                fill="none" stroke="currentColor" strokeWidth="2.5">
        ...
        <div className="lov-table-wrap">
          <table className="lov-table">
            <colgroup>
              ...
              <col className="col-check" />  {/* ย้ายมาขวาสุด */}
            </colgroup>
            <thead>
              <tr>
                {[
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={4} className="lov-loading">
                    <span className="lov-spinner" /> Loading…
                  </td>
                </tr>
              ) : sorted.length === 0 ? (
                <tr>
                  <td colSpan={4} className="lov-empty">No results found</td>
                </tr>
              ) : (
                sorted.map((row) => {
