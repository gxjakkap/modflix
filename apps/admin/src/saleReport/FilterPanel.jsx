...
export default function FilterPanel({
  ...
  //   setProduct(`${row.code} ${row.name}`);
  // };

  // แสดง array selected ทั้งหมด
  // const handleSelectProduct = (selectedArray) => {
  //   if (selectedArray && selectedArray.length > 0) {
  //     // ดึงเฉพาะชื่อ (name) ของทุกอันมาต่อกันด้วยลูกน้ำ
  //     const displayString = selectedArray.map(item => item.name).join(", ");
  ...
  const handleSelectProduct = (selectedArray) => {
    ...
    if (selectedArray && selectedArray.length > 0) {
      ...
      // นำข้อความที่ผ่านเงื่อนไขแล้วไปเซ็ตลง Input
      setProduct(displayString);
    } else {
      // ถ้าไม่ได้เลือกอะไรเลย ให้ล้างค่า Input
      setProduct("");
    }
  };
