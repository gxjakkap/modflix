...
export default function CastEditPage({ pic, username, casts = [], products = [], onSave }) {
    const { code } = useParams()
    const navigate = useNavigate()
    const original = casts.find(c => c.code === code)

    const [draft, setDraft]   = useState(original ? { ...original, cast: [...(original.cast || [])] } : null)
    ...
    const saveEdit = () => {
        setDraft(prev => {
            const updated = [...prev.cast]
            updated[editingIdx] = editingMember
            return { ...prev, cast: updated }
        })
        setEditingIdx(null)
    }

    const linkedProduct = products.find(p => p.code === draft.movieCode)
