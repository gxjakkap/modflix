    ...
    api.admin.customers.list
      ...
      .then((res) => {
        ...
        const formattedData = res.data.data.map((d) => ({
          ...d,
          dateRegistered: new Date(d.dateRegistered).toLocaleDateString(
            "th-TH",
            { dateStyle: "short" },
          ),
  ...
  return (
    <TempPage
      ...
      onDelete={(row) => {
        if (
          window.confirm(
            `You are ${row.isBanned ? "un" : ""}banning user ${row.username}. Are you sure?`,
          )
        ) {
          if (row.isBanned) {
                  ...
                  .then((res) => {
                    ...
                    const formattedData = res.data.data.map((d) => ({
                      ...
                      dateRegistered: new Date(
                        d.dateRegistered,
                      ).toLocaleDateString("th-TH", { dateStyle: "short" }),
                    })) as unknown as Customer[];
